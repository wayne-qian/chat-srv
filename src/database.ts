...
export class Table {
    ...
    constructor(baseDir: string, readonly name: string) {
    ...
    json<T extends object>(key: string) {
        const path = Path.join(this.dir, key)
    ...
    bigList<T extends object>(key: string) {
        const path = Path.join(this.dir, key)
...
export class Database {
    ...
    table(name: string) {
        return new Table(this.dir, name)
    }
}

export const db = new Database(Path.join(OS.homedir(), '.chat-db'))